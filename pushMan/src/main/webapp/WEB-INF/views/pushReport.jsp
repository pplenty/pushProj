	...
	<div class="container-fluid">
		<div class="row">
			...
			<div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
				...
				<!-- 검색창 -->
				<form action='pushList.do' method='get'
						...
						<tbody class="campaign">
							...
							<c:forEach items="${list}" var="pushCamp">
								<tr cno="${pushCamp.camp_id}">
									...
									<td style="text-align: center;">${pushCamp.push_wait}</td>
									<!-- 대기 -->
									<td style="text-align: center;">${pushCamp.push_succ}</td>
									<!-- 성공 -->
									<td style="text-align: center;">${pushCamp.push_fail}</td>
									<!-- 실패 -->
									<td style="text-align: center;">${pushCamp.push_open}</td>
									<!-- 오픈 -->
									<!-- 클릭 BEGIN-->
									<c:set var="pushType" value="${pushCamp.camp_reqUid}" />
									<c:if test="${pushType == 'RICH'}">
										<td style="text-align: center; cursor: pointer;"
											class="pushClickReport" cno="${pushCamp.camp_id}"><a
											href="" class="pushClickReport" cno="${pushCamp.camp_id}">${pushCamp.push_click}</a></td>
									</c:if>
									...
									<!-- SMS 발신 BEGIN-->
									<td style="text-align: center;"><c:set var="retargetFlag"
											value="${pushCamp.checkReTarget}" /> <c:if
											test="${retargetFlag == 'Y'}">
										...
										</c:if> <c:if test="${retargetFlag == 'N'}">
											<p>NO</p>
										</c:if></td>
									<!-- SMS 발신 END-->
									<td style="text-align: center;">
										<button class="btn detailBtn" cno="${pushCamp.camp_id}">상세</button>
									</td>
									<td class="push_title" style="display: none;">${pushCamp.push_title}</td>
									<!-- 제목 -->
									<td class="popup_content" style="display: none;">${pushCamp.popup_content}</td>
									<!-- 내용 -->
									<td class="inapp_content" style="display: none;">${pushCamp.inapp_content}</td>
									<!-- 내용 -->
								</tr>
							</c:forEach>
